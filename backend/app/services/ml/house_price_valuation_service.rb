...
module Ml
  class HousePriceValuationService
    ROOT_DIR = Rails.root.join("..").expand_path
    ML_ROOT = ROOT_DIR.join("ml-training")
    ARTIFACTS_DIR = ML_ROOT.join("artifacts", "valuation", "latest")
    INFERENCE_SCRIPT = ML_ROOT.join("infer_valuation.py")
    ...
    def call
      ...
      unless status.success?
        Rails.logger.warn("ML valuation inference failed for property #{@property.id}: #{stderr.presence || stdout}")
        return nil
      end

      JSON.parse(stdout)
    rescue StandardError => e
      Rails.logger.warn("ML valuation inference exception for property #{@property.id}: #{e.class}: #{e.message}")
      nil
    end
