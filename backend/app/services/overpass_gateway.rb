...
class OverpassGateway
  ...
  AMENITY_TYPES  = %w[school pharmacy cafe].freeze
  SHOP_TYPES     = %w[supermarket convenience bakery butcher greengrocer newsagent department_store].freeze
  ...
  def parse_pois(payload)
    Array(payload["elements"]).filter_map do |el|
      ...
      amenity = el.dig("tags", "amenity") || el.dig("tags", "shop") || el.dig("tags", "railway")
